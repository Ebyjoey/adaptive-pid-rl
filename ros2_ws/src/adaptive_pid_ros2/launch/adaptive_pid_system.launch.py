...
def generate_launch_description() -> LaunchDescription:
    env_config_arg = DeclareLaunchArgument(
        "env_config_path", default_value="configs/env/pendulum.yaml",
    ...
    model_path_arg = DeclareLaunchArgument(
        "model_path", default_value="runs/ppo/final_model.zip", description="Path to the trained SB3 model .zip"
    )
    vecnormalize_path_arg = DeclareLaunchArgument(
        "vecnormalize_path", default_value="runs/ppo/vecnormalize.pkl",
        description="Path to the matching VecNormalize stats .pkl"
    )
    seed_arg = DeclareLaunchArgument("seed", default_value="0", description="Random seed for reference/disturbance nodes.")
    ...
    reference_node = Node(
        package="adaptive_pid_ros2", executable="reference_node", name="reference_node", output="screen",
        parameters=[{"env_config_path": env_config_path, "seed": seed}],
    )
    plant_node = Node(
        package="adaptive_pid_ros2", executable="plant_node", name="plant_node", output="screen",
        parameters=[{"env_config_path": env_config_path}],
    )
    disturbance_node = Node(
        package="adaptive_pid_ros2", executable="disturbance_node", name="disturbance_node", output="screen",
        parameters=[{"env_config_path": env_config_path, "seed": seed}],
    )
    pid_controller_node = Node(
        package="adaptive_pid_ros2", executable="pid_controller_node", name="pid_controller_node", output="screen",
        parameters=[{"env_config_path": env_config_path}],
    )
    rl_agent_node = Node(
        package="adaptive_pid_ros2", executable="rl_agent_node", name="rl_agent_node", output="screen",
        parameters=[{
    ...
    logging_node = Node(
        package="adaptive_pid_ros2", executable="logging_node", name="logging_node", output="screen",
        parameters=[{"output_path": "logs/ros2_run.csv"}],
    )
    visualization_node = Node(
        package="adaptive_pid_ros2", executable="visualization_node", name="visualization_node", output="screen",
    )
