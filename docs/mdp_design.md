...
**Empirical note (validated in `tests/integration/test_gym_env.py`):** the Ziegler–Nichols gains computed against the nominal plant (`scripts/autotune_zn.py`) reliably stabilize *that* nominal plant, but classic closed-loop ZN tuning is a quarter-decay-ratio design — deliberately fast rather than conservative — and its gains are fixed regardless of operating conditions. Under this project's full domain-randomization ranges (simultaneous mass, length, damping, and actuator-degradation variation), the fixed ZN baseline fails to stabilize a nontrivial fraction of sampled plant configurations. This is not a tuning mistake to be corrected; it is precisely the gap that motivates adaptive gain scheduling, and is reported as a baseline result rather than papered over.
